...
void physics_update(PHYS_PTR** chunks, struct PHYS_PTR_vec* objects, struct Context* ctx, float dt){
    for(int i = 0; i < objects->size; i++){
        PHYS_PTR object = objects->data[i];
        int chunk_x = floor(object->position[0]/CHUNK_WIDTH);
        int chunk_y = floor(object->position[1]/CHUNK_HEIGHT);
